...
fn main() {
    println!("Hello world!");
    // TODO: build engine functionality; board should be done? 
}
...
mod tests {
    ...
    fn test_bitboard_operations() {
        let mut bb = system::BitBoard::new();
    ...
    fn test_board_conversion() {
        let start_fen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
        let parsed = system::parse_fen(start_fen);
        let mut board: system::Board = parsed.into();

        assert!(matches!(board.pieces[0], Some(system::Piece::WhiteRook)));
        assert!(matches!(board.pieces[63], Some(system::Piece::BlackRook)));

        board.castling_rights.empty();
        assert_eq!(board.castling_rights.0, system::Castling::NO_CASTLING);
        board.castling_rights.all();
        assert_eq!(board.castling_rights.0, system::Castling::ANY_CASTLING);
    } // SUCCESS

    #[test]
    fn test_board_supports() {
        use system::Color::White as white;
        use system::Color::Black as black;
        
        let fen = "rnbqkbnr/pppppppp/8/PPPPPPPP/pppppppp/8/PPPPPPPP/RNBQKBNR b KQkq - 0 1";
        let parsed = system::parse_fen(fen);
        let mut board: system::Board = parsed.into();
        ...
        let parsed = system::parse_fen(no_bishop_pair);
        let board: system::Board = parsed.into();
        assert!(!board.has_bishop_pair(white));
        assert!(!board.has_bishop_pair(black));
    } // SUCCESS
    ...
    fn test_board_indexing() {
        ...
        let white = board.side_pieces(system::Side::WHITE);
        assert_eq!(white.pop_count(), 16);
        let white_knights = board.piece_bb(system::Side::WHITE, system::Piece::WhiteKnight);
